<template>
  <BaseToolPage
    ...
    action-label="Convert to JPG"
    success-message="Conversion Complete!"
    download-filename="page-1.jpg"
    preview-component="pdf"
    @files-selected="handleFilesSelected"
    @process-files="convertToJPG"
  >
        ...
        <q-card class="settings-card bg-dark text-white">
          <q-card-section>
            ...
            <div v-if="pdfMetadata" class="info-box q-mt-md">
              ...
              <span>PDF has {{ pdfMetadata.pageCount }} pages - will create {{ pdfMetadata.pageCount }} JPG file(s)</span>
            </div>
          </q-card-section>
        </q-card>
      </div>
    ...
    <template #result-section>
      <div v-if="convertedImages.length > 0" class="result-section">
        <div class="success-message">
          ...
          <p>Created {{ convertedImages.length }} JPG image(s)</p>
        </div>
...
async function convertToJPG() {
  ...
  try {
    ...
    convertedImages.value = await pdfToImages(file, 'jpeg', dpi.value)
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Failed to convert PDF to JPG'
  }
}
...
async function downloadAllAsZip() {
  ...
  try {
    ...
    convertedImages.value.forEach((image, index) => {
      ...
      zip.file(`page-${index + 1}.jpg`, bytes)
    })
