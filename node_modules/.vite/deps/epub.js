...
var import_promises = (/* @__PURE__ */ __commonJSMin(((exports, module) => {
...
var parseXml$1 = function(xmlData) {
...
//#endregion
//#region node_modules/epub/dist/epub.js
...
function attrsOf(obj) {
	const result = {};
	for (const key of Object.keys(obj)) if (key.startsWith("@_")) result[key.slice(2)] = String(obj[key]);
	return result;
}
...
function extractIdentifiers(val, out) {
	...
	if (scheme) out[scheme] = contents;
	else if (id && id.match(/uuid/i)) out.UUID = contents.replace("urn:uuid:", "").toUpperCase().trim();
}
var EPub = class {
	input;
	imageroot;
	...
	constructor(input, imageroot, linkroot) {
		...
		if (!this.imageroot.endsWith("/")) this.imageroot += "/";
		if (!this.linkroot.endsWith("/")) this.linkroot += "/";
	}
	async parse() {
		...
		if (this.spine.toc) await this._parseTOC();
	}
	...
	async _open() {
		try {
			const buf = typeof this.input === "string" ? await (0, import_promises.readFile)(this.input) : this.input;
			this.zip = await import_jszip_min.default.loadAsync(buf);
		} catch {
	...
	async _checkMimeType() {
		for (const name of Object.keys(this.zip.files)) if (name.toLowerCase() === "mimetype") {
			this.mimeFile = name;
			break;
		}
		if (!this.mimeFile) throw new Error("No mimetype file in archive");
		if ((await this._readFile(this.mimeFile)).toString("utf-8").toLowerCase().trim() !== "application/epub+zip") throw new Error("Unsupported mime type");
	}
	async _getRootFiles() {
		for (const name of Object.keys(this.zip.files)) if (name.toLowerCase() === "meta-inf/container.xml") {
			this.containerFile = name;
			break;
		}
		if (!this.containerFile) throw new Error("No container file in archive");
		const rootfiles = parseXml((await this._readFile(this.containerFile)).toString("utf-8").trim()).rootfiles;
		if (!rootfiles || !rootfiles.rootfile) throw new Error("No rootfiles found");
		for (const rf of asArray(rootfiles.rootfile)) if (String(rf["@_media-type"]).toLowerCase() === "application/oebps-package+xml" && rf["@_full-path"]) {
	...
	async _handleRootFile() {
		return parseXml((await this._readFile(this.rootFile)).toString("utf-8"));
	}
	_parseRootFile(rootfile) {
		...
		for (const fullKey of Object.keys(rootfile)) {
			...
			switch ((fullKey.split(":").pop() || "").toLowerCase().trim()) {
				case "metadata":
				...
				case "spine":
					...
					break;
				case "guide": this._parseGuide(rootfile[fullKey]);
			}
		}
	}
	_parseMetadata(metadata) {
		for (const fullKey of Object.keys(metadata)) {
			...
			switch (key) {
				...
				case "description":
				case "date":
					if (Array.isArray(metadataValue)) this.metadata[key] = textOf(metadataValue[0]);
					else this.metadata[key] = textOf(metadataValue);
					break;
				case "language":
					if (Array.isArray(metadataValue)) this.metadata.language = textOf(metadataValue[0]).toLowerCase();
					else this.metadata.language = textOf(metadataValue).toLowerCase();
					break;
				case "subject": {
					const subjects = asArray(metadataValue);
					if (subjects.length === 0) this.metadata.subject = "";
					else {
				...
				case "creator":
					if (Array.isArray(metadataValue)) {
						...
						const fileAs = typeof metadataValue === "object" && metadataValue != null && metadataValue["@_opf:file-as"];
						this.metadata.creatorFileAs = String(fileAs || this.metadata.creator).trim();
					}
					break;
				case "identifier":
					for (const v of asArray(metadataValue)) extractIdentifiers(v, this.metadata);
					break;
				case "source": {
		...
		for (const meta of asArray(metadata.meta)) {
			...
			if (name) this.metadata[name] = content;
			if (meta["#text"] && property) this.metadata[property] = meta["#text"];
		}
	}
	_parseManifest(manifest) {
		...
		for (const item of asArray(manifest.item)) {
			...
			if (element.href && element.href.substring(0, pathStr.length) !== pathStr) element.href = path.concat([element.href]).join("/");
			if (element.id) this.manifest[element.id] = element;
		}
	}
	_parseGuide(guide) {
		const path = this.rootFile.split("/");
	...
	async _parseTOC() {
		...
		path.pop();
		const idList = {};
		for (const key of Object.keys(this.manifest)) idList[this.manifest[key].href] = key;
		const xml = (await this._readFile(tocHref)).toString("utf-8");
		let result;
		try {
			result = parseXml(xml);
		} catch (err) {
			throw new Error("Parsing container XML failed in TOC: " + (err instanceof Error ? err.message : String(err)));
		}
		const navMap = result.navMap;
		if (navMap?.navPoint) this.toc = this.walkNavMap(navMap.navPoint, path, idList);
	}
	walkNavMap(branch, path, idList, level = 0) {
		...
		for (const item of items) {
			...
			if (navLabel) {
				let title = "";
				if (typeof navLabel.text === "string") title = navLabel.text.trim();
				let order = Number(item["@_playOrder"] || 0);
				if (isNaN(order)) order = 0;
				let href = "";
				const content = item.content;
				if (typeof content?.["@_src"] === "string") href = content["@_src"].trim();
				let element = {
					level,
					order,
					title,
			...
			if (item.navPoint) output.push(...this.walkNavMap(item.navPoint, path, idList, level + 1));
		}
		return output;
	}
	async getChapter(id) {
		...
		s = s.replace(/(\ssrc\s*=\s*["']?)([^"'\s>]*?)(["'\s>])/g, (_o, a, b, c) => {
			...
			let element;
			for (const k of keys) if (this.manifest[k].href === img) {
				element = this.manifest[k];
				break;
			}
			if (element) return a + this.imageroot + element.id + "/" + img + c;
			return "";
		});
		s = s.replace(/(\shref\s*=\s*["']?)([^"'\s>]*?)(["'\s>])/g, (_o, a, b, c) => {
			...
			let element;
			for (const k of keys) if (this.manifest[k].href.split("#")[0] === link) {
				element = this.manifest[k];
				break;
			}
	...
	async getChapterRaw(id) {
		if (!this.manifest[id]) throw new Error("File not found");
		const mediaType = this.manifest[id]["media-type"];
		if (mediaType !== "application/xhtml+xml" && mediaType !== "image/svg+xml") throw new Error("Invalid mime type for chapter");
		const data = await this._readFile(this.manifest[id].href);
		return data ? data.toString("utf-8") : "";
	}
	async getImage(id) {
		if (!this.manifest[id]) throw new Error("File not found");
		if (!(this.manifest[id]["media-type"] || "").toLowerCase().trim().startsWith("image/")) throw new Error("Invalid mime type for image");
		return this.getFile(id);
	}
	async getFile(id) {
		if (!this.manifest[id]) throw new Error("File not found");
		return {
			data: await this._readFile(this.manifest[id].href),
			mimeType: this.manifest[id]["media-type"]
		};
	}
